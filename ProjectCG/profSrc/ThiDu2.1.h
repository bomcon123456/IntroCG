...
void drawCircle(GLintPoint point, float radius) {
	...
	for (int k = 0; k <= n; k++, angle += angleInc) {
		drawLine(savex, savey, point.x + radius * cos(angle), point.y + radius * sin(angle));
		savex = point.x + radius * cos(angle);   savey = point.y + radius * sin(angle);
	}
}
void myDisplay(void) {
...
void myReshape(int w, int h) {// window is reshaped 
	...
	glLoadIdentity();
	gluOrtho2D(0.0, 640.0, 0.0, 480.0);// map drawing region to viewport 
	glMatrixMode(GL_MODELVIEW);// map drawing region to viewport 
...
void run(int argc, char **argv) {
	...
	glutReshapeFunc(myReshape); // xác định viewport
